...
setInterval(function () {
	if (ssince(last_ping) > 2.4) if (window.socket) ping(true);
}, 3200);
...
function preview_all(element) {
	...
	for (var i = 0; i < 3; i++) for (var j = 0; j < 4; j++) disappearing_clone(element, { i: i, j: j, x: get_x(element) + i * 40, y: get_y(element) + j * 40, stay: true, alpha: 1, border: true });
}
...
function booster_modal_logic() {
	...
	for (var i = 1; i <= 5; i++) $(".bitem" + i).html(item_container({ skin: G.items.goldbooster.skin_a }, { level: i, name: "goldbooster", expires: future_s(999999999) }));
}
...
function show_game_guide() {
	if (gameplay == "hardcore")
		show_modal($("#hardcoreguide").html(), { styles: "" }); // background: #E5E5E5; color: #010805;
	else render_guide();
}
...
function rebuild_chat() {
	...
	if (game_chats.length > 250) {
		...
		for (
			var i = 0;
			i < game_chats.length;
			i++ // truncate unimportant stuff more
		) {
	...
	game_chats.forEach(function (log) {
		...
		if (rrpt > 1) rrpt = " <span style='color:" + rcolor + "'>[" + rrpt + "]</span>";
		else rrpt = "";
...
function add_chat(owner, message, color, id) {
	...
	if (game_chats.length > 250 || deletion)
		// previously 240-180|150 - previously 360|180-270 [27/07/18]
		rebuild_chat();
	...
	if (rpt > 1) rpt = " <span style='color:" + rcolor + "'>[" + rpt + "]</span>";
	else rpt = "";
...
function get_input(args) {
	if (!args) return;
	var html = "<div style='" + ((!args.no_wrap && "border: 5px solid gray; padding: 5px; background: black") || "") + "'>",
		ilast = 0,
		focus = null;
	...
	args.elements.forEach(function (element) {
		...
		if (element.button) {
			...
			html +=
				...
				((element.small && "gamebutton-small mb2") || "mb5") +
	...
	if (name == "use_hp" || name == "hp") {
		...
		if (is_array(target)) {
			...
			var hostiles = get_nearby_hostiles({ range: character.range - 2, limit: 12 }),
				targets = [],
				mp = character.mp - 200,
				hmp = parseInt(mp / hostiles.length);
...
function on_skill(key, event) {
	...
	if (skill.type == "item") {
		...
		for (i = character.items.length - 1; i >= 0; i--) {
			if (character.items[i] && character.items[i].name == skill.name) {
				num = i;
				break;
			}
		}
...
function inventory_click(num, event) {
	...
	if (event && (event.which == 2 || event.button == 4)) {
		...
		if (last_invclick && last_invclick == num + iname && $(".inventory-item").html().length) $(".inventory-item").html("");
		else if (character.items[num]) {
			if (character.items[num].name == "placeholder") return;
			if (character.items[num].name == "computer" || character.items[num].name == "supercomputer") {
				// gameplay=="hardcore" &&
...
function start_character_runner(name, code_slot_or_name) {
	...
	$("#iframelist").append(
		'<div class="clickable" onclick="show_character_snippet(\'' +
			...
			'\')"><iframe src="' +
			url +
			"?no_html=true&is_bot=1&code=" +
			code_slot_or_name +
			'" id="' +
			rid +
			'" style="border: 5px solid gray; background-color: black; margin-top: -5px; height: 60px; width: 128px; overflow: hidden; pointer-events: none" data-name="' +
			name +
			'"></iframe></div>',
	);
...
function start_runner(rid, code) {
	...
	$(".dengagebutton").show();
	$(".iengagebutton").hide();
	$(".idengagebutton").css("display", "inline-block");
	$("#" + rid).remove();
	if (no_html) $("body").append('<iframe src="/runner" id="' + rid + '" style="border: none; height: 1px; width: 1px; overflow: hidden; pointer-events: none"></iframe>');
		...
		$("#iframelist").append(
			'<div class="clickable" onclick="show_snippet()"><iframe src="/runner" id="' +
				rid +
				'" style="border: 5px solid gray; background-color: black; margin-top: -5px; height: 60px; width: 128px; overflow: hidden; pointer-events: none"></iframe></div>',
		);
	$("#iframelist").css("display", "inline-block");
	code_run = true;
...
function set_setting(owner, key, value) {
	var data = storage_get("settings_cache"),
		the_code = "",
		to_run = false;
...
function leave_references(sprite) {
	...
	(function () {
		var xx = sprite.real_x,
			yy = sprite.real_y,
			wwidth = sprite.awidth || 10,
			hheight = sprite.aheight || 10;
...
function remove_sprite(sprite) {
	...
	try {
		...
		console.log("Sprite is orphan, can't remove. Type: " + sprite.type);
	}
...
function destroy_sprite(sprite, mode) {
	...
	try {
		...
		console.log("Couldn't destroy sprite: " + sprite.type + " because: " + e);
	}
}
...
function buy(name, quantity) {
	if (G.items[name] && G.items[name].cash && !G.items[name].p2w && G.items[name].cash <= character.cash) return buy_with_shells(name, quantity);
...
function code_eval_if_r(code) {
	code_active &&
		...
		document.getElementById("maincode").contentWindow &&
		document.getElementById("maincode").contentWindow.eval &&
		document.getElementById("maincode").contentWindow.eval(code);
}
...
function exchange_animation_logic() {
	...
	if (mssince(last_excanim) > 300) {
		...
		$("#eitem")
			.children()
			.css("border-color", exccolors[exclast % exccolors.length]);
...
function set_uchance(dchance, html) {
	...
	var chance = min(9999, parseInt(dchance * 10000) || 0),
		first = "" + parseInt(chance / 100),
		last = "" + parseInt(chance % 100);
	chance = chance / 100;
...
function compound_animation_logic() {
	...
	if (character.map == "main" && Math.random() < 0.4)
		random_spark({ map: "main", x: G.maps.main.ref.c_mid[0] + 5 - Math.random() * 10, y: G.maps.main.ref.c_mid[1] - Math.random() * 6 + 1, in: "main" }, { color: "spark" });
}
...
function upgrade_animation_logic() {
	...
	if (character.map == "main" && Math.random() < 0.4)
		random_spark({ map: "main", x: G.maps.main.ref.u_mid[0] + 5 - Math.random() * 10, y: G.maps.main.ref.u_mid[1] - Math.random() * 6 + 1, in: "main" }, { color: "spark" });
}
...
function reopen() {
	...
	draw_trigger(function () {
		...
		if (u_retain && u_retain_t && rendered_target == "upgrade" && !character.q.upgrade) {
			...
			for (var i = 0; i < 3; i++) if ((u_retain[i] || u_retain[i] === 0) && character.items[u_retain[i]]) on_rclick($("#citem" + u_retain[i])[0]);
...
function generate_textures(name, stype) {
	...
	if (stype == "item") {
		...
		} else textures["item" + name][0] = textures["item" + name][1];
	}
	...
	if (stype == "animation") {
		...
		var width = PIXI.utils.BaseTextureCache[a.file].width,
			fwidth = Math.floor(width / a.frames);
	...
	if (stype == "static") {
		...
		if (!texture) {
			var position = G.positions[skin],
				file = G.tilesets[position[0]].file;
...
function recreate_wtextures() {
	(window.wtextures || []).forEach(function (t) {
		if (t) t.destroy();
	});
	...
	for (var last_water_frame = 0; last_water_frame < 4; last_water_frame++) {
		var wsprite = new PIXI.extras.TilingSprite(
			textures[wtile_name][last_water_frame],
			wtile_width / scale + 4 * textures[wtile_name][0].width,
			wtile_height / scale + 4 * textures[wtile_name][0].height,
		);
...
function recreate_dtextures() {
	...
	for (var last_water_frame = 0; last_water_frame < 3; last_water_frame++) {
		var dsprite = new PIXI.extras.TilingSprite(
			tile_textures[current_map][GEO["default"]][last_water_frame % tile_textures[current_map][GEO["default"]].length],
...
function random_rotating_rectangle(entity, args) {
	...
	var size = random_one([3, 5, 7]),
		x_step = random_one([1, -1, 2, -2]),
		y_step = random_one([-0.5, 0, 1, 2, 3]),
		r_step = random_one([-0.2, 0.2, -0.4, 0.4]);
	e.lineStyle(3, color);
	e.beginFill(color);
...
function random_spark(coord, args) {
	...
	var color = random_one(colors);
	var size = 0.2,
		x_step = random_one([0.25, 0.5, 0, -0.25, -0.5]),
		y_step = random_one([0.25, 0.5, 0.75]),
		a_map = current_map;
	e.lineStyle(3, color);
	e.beginFill(color);
	e.drawRect(-size / 2, -size / 2, size / 2, size / 2);
...
function small_success(entity, args) {
	for (var j = 0; j < 4; j++)
		for (var i = 0; i < 30; i++) {
			draw_timeout(function () {
				random_rotating_rectangle(entity, args);
			}, i * 16);
		}
}
...
function start_emblem(sprite, name, args) {
	if (!args) args = {};
	if (sprite.emblems[name]) {
		sprite.emblems[name].frames = args.frames || 225;
		return;
	}
	var esprite = new_sprite(name, "emblem");
	if (args.no_dip) esprite.frame_list = [0.57, 0.6, 0.63, 0.66, 0.69, 0.72, 0.75, 0.78, 0.82, 0.86, 0.9, 0.95, 1];
...
function start_animation(sprite, name, mode) {
	...
	if (sprite.animations[name]) {
		sprite.animations[name].frame = 0;
		return;
	}
	var asprite = new_sprite(name, "animation"),
		width = (sprite.hitArea && sprite.hitArea.width) || sprite.texture.width,
		height = (sprite.hitArea && sprite.hitArea.height) || sprite.texture.height;
...
function map_animation(name, args) {
	...
	var width = asprite.width,
		height = asprite.height;
	var def = G.animations[name] || {};
	asprite.atype = "map";
	asprite.continuous = true;
	...
	for (var i = 0; i < draw_timeouts.length; i++) {
		...
		if (start >= timeout[1]) {
			...
			try {
				...
				console.log("draw_timeout_error: " + e);
				console.log("code: " + timeout[0]);
				console.log(e.stack);
			}
		}
	}
...
function draw_trigger(f) {
	if (in_draw) {
		try {
			...
			console.log("draw_trigger_error: " + e);
			console.log("code: " + f);
			console.log(e.stack);
		}
		...
		if (tint.type == "skill") {
			...
			if (start > tint.end) {
				...
				if (!tint.added) {
					tint.added = true;
					$(tint.selector).children(".thetint").remove();
					$(tint.selector).append(
						"<div style='position: absolute; " + (tint.pos || "bottom") + ": 0px; left: 0px; right: 0px; height: 1px; background: " + tint.color + "; z-index: 1' class='thetint'></div>",
					);
				}
				var since = mssince(tint.start),
					to = -mssince(tint.end);
...
function get_tint(selector) {
	for (var i = 0; i < tints.length; i++) if (tints[i].selector == selector) return tints[i];
	return null;
}
...
function add_tint(selector, args) {
	...
	args.end = new Date();
	args.end.setMilliseconds(args.end.getMilliseconds() + args.ms);
...
var next_attack = new Date(),
	next_potion = new Date(); // backwards compatibility
...
function border_logic(element) {
	...
	if (element.aborder) {
		...
		element.aborder = null;
	}
	if (element.bborder) {
		destroy_sprite(element.bborder);
		element.bborder = null;
	}
	if (element.cborder) {
...
function name_logic(element) {
	...
	if (!options.show_names && !(options.always_hpn || options.always_names) && (mtarget || xtarget || ctarget) != element && character && element.name_tag) {
		//show_names
		destroy_sprite(element.name_tag, "children");
		element.name_tag = null;
		element.ntag_cache = null;
	} else if (options.show_names || !character || options.always_hpn || options.always_names || (mtarget || xtarget || ctarget) == element) {
		//  && !element.name_tag - added ntag_cache [06/12/16]
...
function add_name_tag_old(element) {
	...
	var name = "Lv." + element.level + " " + element.name,
		border_color = 0x716d6c,
		width = name.length * 4 + 4,
		height = 11;
	...
	if (element.role == "gm") border_color = 0xe6a32f;
...
function add_name_tag(element) {
	...
	if (element.npc);
		...
		(pvp || is_pvp) &&
		character &&
		character.guild != element.guild &&
		character.party != element.party &&
		(element.target == character.name || element.focus == character.name)
	);
	else if (character && character.party && character.party == element.party)
		//red
		bar.color = "#6F3F87"; // purple
	...
	name.anchor.set(0.5, 0);
	var fpl = { fontFamily: SZ.font, fontSize: 64, fill: "#A2AAB0", align: "center" },
		level;
	if (bar.level) {
	...
	name.x = round(width / 2);
	name.y = 1.5;
	name.roundPixels = roundPixels;
	bordered_rectangle.addChild(name);

	if (bar.level) {
		level.x = width + 4.5;
		level.y = 1.5;
		level.roundPixels = roundPixels;
...
function hp_bar_logic(element) {
	...
	if (
		...
		xtarget == element ||
		mtarget == element ||
		(character && character.party && character.party == element.party) ||
		(character && character.party && ((element.target && in_arr(element.target, party_list)) || (element.focus && in_arr(element.focus, party_list)))) ||
		(character && (element.target == character.name || element.focus == character.name)) ||
		(character && character.team && !element.npc) ||
		(character && element.controller == character.name) ||
		options.always_hpn
	) {
		add_hp_bar(element);
	} else if (element.hp_bar) {
		destroy_sprite(element.hp_bar, "children");
		element.hp_bar = null;
		element.hp_color = null;
	}
}

function add_hp_bar_old(element) {
	var width = max(32, round(element.width * 0.8)),
		bsize = 1,
		bsize2 = round(2 * bsize); // bsize = 1.5 -> awesome [12/08/16]
	var color = 0xb12727; //red

	if (element.npc) color = 0xe09400;
	else if (element.type == "character" && !pvp && !is_pvp && (ctarget == element || xtarget == element))
		color = 0x368c2b; // green
		...
		//red
		color = 0x6f3f87; // purple
	else if (character && character.guild && character.guild == element.guild)
		color = 0x3eb6c0; // teal
	else if (character && is_monster(element) && (ctarget == element || xtarget == element));
	else if (character && element.controller == character.name)
		//red
		color = 0x899b92;
...
function add_hp_bar(element) {
	...
	if (character && is_monster(element) && (ctarget == element || xtarget == element));
	else if (mtarget == element)
		//red
		color = hx("#91987B");
	...
	if (level) {
		var lcolor = "#5EC063",
			diff = calculate_difficulty(element);
		...
		var fpl = { fontFamily: SZ.font, fontSize: 64, fill: "white", align: "center" },
			level = new PIXI.Text((element.level > 99 && "XX") || element.level, fpl);
		level.scale = new PIXI.Point(0.125, 0.125);
		level.anchor.set(0.5, 0);
		level.x = -4;
...
function calculate_difficulty(monster) {
	var mdps = monster.attack * monster.frequency,
		dps = 4000,
		hp = 8000,
		mhp = monster.hp;
...
function d_line(start, end, args) {
	...
	} else if (!args.color || args.color == "attack")
...
function step_d_texts(entity, add) {
	...
	for (var i = 0; i < entity.texts.length; i++) {
		...
		var ms = mssince(text.last_fade),
			y = round((4 * ms) / text.anim_time) + (add || 0);
		// console.log(y);
		if (2 < y && y < 7) y = 4;
...
function shift_d_texts(entity, add) {
	for (var i = 0; i < entity.texts.length; i++) {
		var text = entity.texts[i];
		text.y -= add || 0;
		if (entity.texts[i + 1] && text.y - entity.texts[i + 1].y < 10) add = 10 - (text.y - entity.texts[i + 1].y);
		else break;
	}
}
...
function d_text_new(message, entity, args) {
	...
	if (color == "hp") color = "green";
		...
		color = "#317188"; // previously "#006AA9"
	...
	var text = new PIXI.Text(message, {
		fontFamily: SZ.font,
		fontSize: size * text_quality,
		fontWeight: "bold",
		fill: color,
		align: "center",
		dropShadow: true,
		dropShadowColor: "#909090",
	...
	if (entity.texts.length && text.y - entity.texts[0].y < 10) shift_d_texts(entity, 10 - (text.y - entity.texts[0].y));
...
async function api_call_l(method, args, r_args) {
	if (!args) args = {};
	var result = {};
	await api_call(method, args, r_args)
		.then(function (data) {
			result = data;
		})
		.catch(function (data) {
...
function light_logic() {
	...
	if (Dev) console.log(lighting.clearColor);
...
setInterval(function () {
	...
	if (
		(inside == "game" && ssince(last_servers_and_characters) > 89) ||
		(inside == "game" && character && !character.afk && ssince(last_servers_and_characters) > 4) ||
		(inside == "selection" && ssince(last_servers_and_characters) > 4) ||
		(window.is_comm && ssince(last_servers_and_characters) > 4)
	) {
...
function update_servers_and_characters() {
	...
	if (!X.unread) $(".comcount").html("");
	else $(".comcount").html(" [" + X.unread + "]");
...
function handle_information(infs) {
	if (infs.reason) log_error("Fail reason: " + infs.reason);
		...
		if (info.type == "code_list") {
			...
			if (is_electron && electron_is_main()) {
				...
				for (var num in X.codes) {
					if (!info.code_list[num]) file_op_queue[num] = ["remove", num, X.codes[num][1]];
				}
				for (var num in info.code_list) {
					if (!X.codes[num] || !code_ref[num] || X.codes[num][1] > code_ref[num].v) file_op_queue[num] = ["download", num, info.code_list[num][1]];
				}
			}
		...
		var result = JSON.stringify(
			...
			function (key, val) {
				...
				if (val != null && typeof val == "object") {
					...
					seen.push(val);
					if ("x" in val) {
						// amplify - also in common_functions.js safe_stringify
						var new_val = {};
...
jQuery.fn.shtml = function (r) {
	// simplified/processed html for editable div's [26/07/18]
	var e = jQuery(this);
	var val = e.html();
...
jQuery.fn.cfocus = function () {
	// Focusing an empty contenteditable is unreliable at this time [27/08/18]
...
async function save_code_file(num, data, v) {
	// only "code"
	var folder = ide_root + "/adventureland/characters",
		filename = X.codes[num][0] + "." + num + ".js";
	...
	file_op[filename] = new Date();
	await fs.promises.writeFile(folder + "/" + filename, data.toString(), "utf8");
	file_op[filename] = new Date();
	var cdata = filename_to_cdata(filename);
	code_ref[cdata.slot] = cdata;
	code_ref[cdata.slot].v = v;
...
async function download_code_file(num, dynamic) {
	...
	await api_call("load_code", { name: num, run: "", log: false, save: true });
	...
	if (deleted) {
		...
		await fs.promises
			...
			.then(async (data) => {
				...
				if (data) await api_call("save_code", { code: data.toString(), slot: cdata.slot, name: cdata.name, auto: true, electron: true });
			})
			.catch(() => {});
		code_ref[cdata.slot] = cdata;
		code_ref[cdata.slot].v = 1;
		code_ref[cdata.slot].m = new_m;
		flag_new_code[cdata.slot] = new Date();
		await sleep(1200);
	} else if (code_ref[cdata.slot] && !X.codes[cdata.slot]) {
		// deleted file
		...
		await sleep(1200);
	} else if (code_ref[cdata.slot] && (code_ref[cdata.slot].m != new_m || cdata.name != X.codes[cdata.slot][0] || X.codes[cdata.slot][1] != code_ref[cdata.slot].v)) {
		...
		if (!code_ref[cdata.slot] || X.codes[cdata.slot][1] > code_ref[cdata.slot].v) {
			...
			await fs.promises
				...
				.catch(() => {});
			code_ref[cdata.slot] = cdata;
			code_ref[cdata.slot].v = X.codes[cdata.slot][1];
			code_ref[cdata.slot].m = new_m;
		}
		await sleep(1200);
	...
	try {
		...
		if (!fs)
			try {
				...
				console.error("fs-extra import issue");
				fs = require("fs");
			}
		ide_root = path.join(electron.remote.app.getPath("appData"), electron.remote.app.getName());
		await fs.promises.access(ide_root + "/autosync" + user_id).catch(async function () {
			await fs.promises.mkdir(ide_root + "/autosync" + user_id);
		});
		ide_root = ide_root + "/autosync" + user_id;
		await fs.promises.access(ide_root + "/adventureland").catch(async function () {
			await fs.promises.mkdir(ide_root + "/adventureland");
		});
		await fs.promises.access(ide_root + "/adventureland/characters").catch(async function () {
			await fs.promises.mkdir(ide_root + "/adventureland/characters");
		});
		await fs.promises.access(ide_root + "/adventureland/codes").catch(async function () {
			await fs.promises.mkdir(ide_root + "/adventureland/codes");
		});
		await fs.promises.access(ide_root + "/adventureland/libraries").catch(async function () {
			await fs.promises.mkdir(ide_root + "/adventureland/libraries");
		});
		await fs.promises.access(ide_root + "/history").catch(async function () {
			await fs.promises.mkdir(ide_root + "/history");
		});
		await fs.promises
			.readFile(ide_root + "/data.json", "utf8")
			.then((data) => {
				code_ref = JSON.parse(data) || {};
			})
			.catch(() => {
				code_ref = {};
			});
		...
		await fs.promises
			.readdir(ide_root + "/adventureland/characters")
			.then(async (files) => {
